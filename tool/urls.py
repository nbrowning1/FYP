...
urlpatterns = [
    ...
    url(r'^password-change-done/$',
        auth_views.PasswordChangeDoneView.as_view(template_name='tool/auth/password_change_done.html'),
    ...
    url(r'^password-reset-done/$',
        auth_views.PasswordResetDoneView.as_view(template_name='tool/auth/password_reset_done.html'),
        name='password_reset_done'),

    url(r'^password-reset-confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        auth_views.PasswordResetConfirmView.as_view(template_name='tool/auth/password_reset_confirm.html',
                                                    success_url=reverse_lazy('tool:password_reset_complete')),
        name='password_reset_confirm'),

    url(r'password-reset-complete/$',
        auth_views.PasswordResetCompleteView.as_view(template_name='tool/auth/password_reset_complete.html'),
