...
class AttendanceSessionRowTest(TestCase):
  ...
  def test_row_invalid_session_format(self):
    ...
    self.assertEqual(data.get_error_message(), 'Expected newline to separate date and session id for: 31/01/2017 - Second Session ID at column 2')
    
  def test_row_invalid_date_format(self):
    # wrong formatting
    data = AttendanceSessionRow(['Device ID(s)', '01/01/2017\nFirst Session ID', '31-01-2017\nSecond Session ID'])
    self.assertEqual(data.get_error_message(), 'Incorrect date format: 31-01-2017, should be DD/MM/YYYY at column 2')
  ...
  def test_row_empty(self):
    ...
    # no session id
    data = AttendanceSessionRow(['Device ID(s)', '01/01/2017\n'])
    self.assertEqual(data.get_error_message(), 'Expected newline to separate date and session id for: 01/01/2017\n at column 1')
...
class AttendanceRowTests(TestCase):
  ...
  def test_invalid_multiple(self):
    ...
    self.assertEqual(data.get_error_message(), 'Unrecognised student: B00000000, Unrecognised attendance value for B00000000: donkey at column 2')
  ...
  def test_incorrect_column_count(self):
    ...
    # too many
    data = setup_input_data_attendance(attendance_session_row, 'B00112233', 'Y', 'N')
    self.assertEqual(data.get_error_message(), 'Number of data columns doesn\'t match number of sessions. Expected 1 but found 2')
    
    # too few
    data = setup_input_data_attendance(attendance_session_row, 'B00112233')
    self.assertEqual(data.get_error_message(), 'Number of data columns doesn\'t match number of sessions. Expected 1 but found 0')
...
def setup_test_student(device_id):
  student_user = User.objects.create_user(username='test', email='test@email.com', password='CorrectHorseBatteryStaple')
